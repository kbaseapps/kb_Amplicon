...
class MDSUtils:
    ...
    def _mds_on_matrix(self, input_obj_ref, n_components, distance_metric='bray_curtis',
                        p_metric=False, p_max_iter=300, p_eps=1e-9, dissimilarity='precomputed'):
        """
        _mds_on_matrix: perform MDS analysis on a matrix object by calling an R script
        ...
        X_nmds = clf_nmds.fit_transform(distance_mat, init=X_mds)

        return {'X_transformed': X_nmds,
                'Distance_matrix': distance_mat,
                'Estimator_params': clf_nmds.get_params()}
